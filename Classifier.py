...
def getListofFeatures(myfile):
    '''
    getListofFeatures(string) -> List
...
def getRootPrediction(myfile, mymodel, outfile):
    ...
    try:
        ...
        #outFile = 'signal_ml.root'
        nEvents = 0
        fRun = FairRunAna()
        fRun.SetInputFile('signal_pid.root')
        fRun.SetOutputFile(outfile)
        fRun.SetGenerateRunInfo(kFALSE)
        
        myFile = TFile('signal_pid.root')
...
def PlotCorrMatrix(myfile):
    '''
    PlotCorrMatrix(string) -> None
    
    A method used to plot the correlation between input features to the Ml algorithm.
    '''
    df = loadmyfile(myfile)
...
def PlotFeature(myfile, feature, bins=10, range=(0,1), ylog=False):
    '''
    PlotFeature(string, string, integer=10, range=(0,1), ylog=False) -> None
    
    A method used to plot a histogram of the feature of interest. The last three arguments are optional, the 
    range argument accepts only tuples (min,max) which specifies the limits of the histogram.
    An example call is illustrated
    
    >>> PlotFeature('signal_pid.root', 'momentum')
    '''
    df = loadmyfile(myfile)
    feature = str(feature)
    bins = int(bins)
    variable = df.loc[:,feature]
    plt.hist(df.loc[:,feature], bins=bins, range=range)
    plt.xlabel(feature)
    plt.ylabel('counts')
    if (ylog==True):
        plt.yscale('log')
...
def PlotScatter(myfile, feature_1, feature_2):
    '''
    PlotScatter(string, string, string) -> None
    
    A method used to make a scatter plot between input two features. An example call is illustrated
    ...
    var_1 = df.loc[:,feature_1]
    var_2 = df.loc[:,feature_2]
